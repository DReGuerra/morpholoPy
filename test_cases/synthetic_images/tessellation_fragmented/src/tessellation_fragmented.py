...
axs[0,0].imshow(img_tessellation_fragmented, cmap='gray') # original image
axs[0,0].set_title("(a)", loc='left', fontsize=TITLEFONT)
axs[0,0].axis("off")

axs[0,1].imshow(np.log(psd2D)) # log(PSD)
...
axs[1,1].plot(1/lam[:rasp_length],rasp_norm_au) # radially averaged PSD spatial frequency
